...
type LessThan func(a, b int) bool
...
type timSortHandler struct {
	 ...
	 */
	tmp []int // Actual runtime type will be Object[], regardless of int
 ...
 */
func newTimSort(a []int, lt LessThan) (h *timSortHandler) {
...
func IntSort(a []int, lt LessThan) (err error) {
	lo := 0
	hi := len(a)
	nRemaining := hi
 ...
 */
func binarySort(a []int, lo, hi, start int, lt LessThan) (err error) {
...
/**
  ...
  *          the specified array
*/
func countRunAndMakeAscending(a []int, lo, hi int, lt LessThan) (int, error) {
 ...
 *    should follow it.
 */
func gallopLeft(key int, a []int, base, len, hint int, c LessThan) (int, error) {
 ...
 * @return the int k,  0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
 */
func gallopRight(key int, a []int, base, len, hint int, c LessThan) (int, error) {
	if len <= 0 || hint < 0 || hint >= len {
		return 0, errors.New(" len > 0 && hint >= 0 && hint < len;")
	}
...
func IntBinarySearch(sorted []int, item int, lt LessThan) int {
...
func IntIndexOf(sorted []int, item int, lt LessThan) int {
...
func IntContains(sorted []int, item int, lt LessThan) bool {
	i := IntBinarySearch(sorted, item, lt)
	return !lt(sorted[i], item) && !lt(item, sorted[i])
}

func IntInsert(sorted []int, item int, lt LessThan) []int {
	i := IntBinarySearch(sorted, item, lt)
...
func IntRemove(sorted []int, item int, lt LessThan) []int {
	i := IntBinarySearch(sorted, item, lt)
	if !lt(sorted[i], item) && !lt(item, sorted[i]) {
